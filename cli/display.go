...
func formatDuration(d time.Duration) string {
	...
	return strconv.Itoa(minutes) + "m" + leftPadCLI(strconv.Itoa(seconds), 2) + "s"
}

func leftPadCLI(value string, width int) string {
	for len(value) < width {
		value = "0" + value
	}
	return value
...
func formatAggregateCompact(value any) string {
	if value == nil {
		return ""
	}
	data, err := json.Marshal(value)
	...
	for _, key := range sortedKeys(decoded) {
		...
		case float64:
			parts = append(parts, key+"="+formatFloatCompact(v))
		case bool:
			parts = append(parts, key+"="+strconv.FormatBool(v))
		case map[string]any:
			parts = append(parts, formatNestedAggregate(key, v)...)
		}
	}
	if len(parts) == 0 {
		return string(data)
...
func formatTable(columns []string, rows [][]string, width int) string {
	...
	renderRow := func(row []string) string {
		...
		for i := range widths {
			...
			cells[i] = " " + padRightCLI(value, widths[i]) + " "
		}
...
func formatAggregateRows(rows [][2]string, width int) string {
	...
	for i := 0; i < len(rows); i += cols {
		...
		for _, row := range rows[i:end] {
			cell := padRightCLI(row[0], labelWidth) + " " + padLeftCLI(row[1], valueWidth)
			cells = append(cells, padRightCLI(cell, cellWidth))
		}
...
func padLeftCLI(value string, width int) string {
...
func formatNestedAggregate(name string, values map[string]any) []string {
	...
	for _, key := range sortedKeys(values) {
		...
		if mean, ok := nested["mean"].(float64); ok {
			parts = append(parts, key+".mean="+formatFloatCompact(mean))
		}
	}
	return parts
}
